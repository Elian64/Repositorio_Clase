...
fun main() {
    ...
    val conexion = DriverManager.getConnection(ruta,"user","password")

    println(conexion.isValid(0))*/
        ...
        companion object {
            ...
            private const val CREATE_TABLE_BOOKS_SQL =
                "CREATE TABLE BOOKS ( " +
                        ...
                        "PRIMARY KEY (id))"
            private const val INSERT_BOOKS_SQL =
                "INSERT INTO BOOKS" + "  (author, title, genre, price, publish_date,description ) VALUES " + " (?, ?, ?, ?, ? ,?);"
            private const val SELECT_BOOK_BY_ID =
                "SELECT id,author, title, genre, price, publish_date,description from BOOKS where id =?"
            ...
            private const val UPDATE_BOOKS_SQL =
                "update BOOKS set author = ?,title= ?, genre =? , price = ? , publish_date = ? , description = ? where id = ?;"
        }
