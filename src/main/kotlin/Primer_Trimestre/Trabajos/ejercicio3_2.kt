...
class Persona(
    ...
    constructor(nombre: String, edad: Int, sexo: Char, DNI: String) : this(nombre, edad, DNI, sexo) {

    }

    fun calcularIMC(): String { //no olvidar pasar a las medidas que pide

        when {
            (peso / (altura.pow(2))) < 20 -> return "-1"
            (peso / (altura.pow(2))) >= 20 && (peso / (altura.pow(2))) <= 25 -> return "0"
            (peso / (altura.pow(2))) > 25 -> return "1"

        }
        return "Peso no permitido"
    }
