...
class Modulo(var numeroAlumnos: Int = 15) {
    ...
    fun hayAlumnosConDiez(evaluacion: String): Boolean {
        // var alumnoDiez = notas.count { it[evaluacion.toInt()].equals(10) }
    ...
    fun notaMedia(evaluacion: String): Float { //Lo hace bien si todas las notas estan introducidas
        ...
        // var notaMedia= notas[evaluacion.toInt()].takeIf { !it[evaluacion.toInt()].equals(-1.0F) }?.average()?.toFloat()

        var notaMedia = notas[evaluacion.toInt()].average().toFloat()
        return notaMedia
    }

    fun listaNotasOrdenados(evaluacion:String)/*: List<Alumno> */{ //tendría que ser al revés
    ...
    override fun toString(): String {
        ...
        // return "${alumnos.contentToString()}"
        return "${alumnos[0]?.nombre + ", " + alumnos[0]?.ap1 + " " + alumnos[0]?.ap2}"
